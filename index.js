  ...
  DiscordUtils = {
    ...
    formatButtons(buttons) {
      ...
      if (buttons.length > 25)
        throw new Error(
          "You can only have 25 buttons and 5 action rows per message."
        );
    ...
    formatSelects(custom_id, placeholder, min_values, max_values, options) {
      ...
      if (options.length > 100)
        throw new Error(
          "You can only have 100 options and 5 action rows per message."
        );
    ...
    parseMilliseconds(
      ...
      for (const [name] of Object.entries(options)) {
        if (
          ...
          throw new TypeError(`options.${name} is not a valid option.`);
      }
      // Check if options has any properties that are not booleans
      for (const [name, value] of Object.entries(options)) {
        if (typeof value !== "boolean")
          throw new TypeError(`options.${name} must be a boolean.`);
      }
